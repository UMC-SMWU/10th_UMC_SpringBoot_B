...
public class JwtUtil {
    ...
    // 토큰 생성
    private String createOauthToken(AuthMember member, Duration expiration) {
    ...
    /** 토큰에서 UID 가져오기 (OAuth 로그인용)
     *
     * @param token 유저 정보를 추출할 토큰
     * @return 유저 UID를 토큰 Subject에서 추출합니다
     */
     ...
     *
     * @param token 유저 정보를 추출할 토큰
     * @return 유저 소셜 로그인 타입을 추출한다
     */
    public Member.SocialType getSocialType(String token) {
        try {
            return Member.SocialType.valueOf(getClaims(token).getPayload().get("social_type").toString().toUpperCase());
        } catch (JwtException e) {
            return null;
        }
    }
